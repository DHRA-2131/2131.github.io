...
An observant reader might have noticed that your Empty Pros Project we made in [creating-a-new-pros-project.md](../getting-started/creating-a-new-pros-project.md "mention") doesn't have a `main()` function in `src/main.cpp`. This is because it is hidden away in the PROS source code as PROS needs to initialize it's own code before user code starts (like starting [FreeRTOS](https://www.freertos.org/) which controls how threading works). Below is an annotated fresh project that explains what the functions `main()` is replaced with. If you don't understand everything yet don't worry. &#x20;

**TL;DR:** your code will run the [`initialize()`](#user-content-fn-1)[^1] function at program start and then transition to [`opcontrol()`](#user-content-fn-2)[^2], [`disabled()`](#user-content-fn-3)[^3], or [`autonomous()`](#user-content-fn-4)[^4].&#x20;
...
* [Vexide SDK Reference](https://github.com/vexide/vex-sdk) - This is useful if you'd like to write custom code that doesn't rely on PROS, Vexide, or VexCode. **This is not at all recommended for beginning programmers!**&#x20;
...
[^1]: **Runs immediately\* on program start.**

    * It is advised to put robot initialization code here (Starting a brain screen, reset position tracking, etc.)



    \*Technically this will run after PROS is initialized from the actual entry point.&#x20;

[^2]: Runs when a Competition Switch or Field Control is set to the any of the following condition(s):&#x20;

    * Driver _**AND**_ Enabled
    * No Field Control Detected

[^3]: Runs when a Competition Switch or Field Control is set to any of the following condition(s):&#x20;

    * Driver _**AND**_ Disabled
    * Autonomous _**AND**_ Disabled

[^4]: Runs when a Competition Switch or Field Control is set to any of the following condition(s):&#x20;

    * Autonomous _**AND**_ Enabled
